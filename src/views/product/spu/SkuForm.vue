<template>
    <el-form label-width="100px">
        <el-form-item label="SKU名称">
            <el-input type="" placeholder="SKU名称" v-model="skuParams.skuName"></el-input>
        </el-form-item>
            ...
            <el-form :inline="true">
                <el-form-item v-for="(item, index) in saleArr" :key="item.id" :label="item.saleAttrName">
                    <el-select v-model="item.saleIdAndValueId">
                        <el-option :value="`${item.id}:${saleAttrValue.id}`" v-for="(saleAttrValue, index) in item.spuSaleAttrValueList" :key="saleAttrValue.id" :label="saleAttrValue.saleAttrName"></el-option>
                    </el-select>
                </el-form-item>
            </el-form>
        </el-form-item>
...
const save =async() =>{
    ...
    skuParams.skuAttrValueList = saleArr.value.reduce((prev:any, next:any)=>{
        if(next.saleIdAndValueId){
            let [saleAttrId, skuDefaultImg] = next.saleIdAndValueId.split(':');
            prev.push({
                saleAttrId,
                skuDefaultImg
            });
    ...
    if(result.code==200){
        ...
        ElMessage({
            type: 'error',
            message: '添加SKU失败'     //////////8.1未验证成功，1.添加失败 2.网络出现问题
        })
    }
    //成功
    //失败
